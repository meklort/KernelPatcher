...
{
    register_kernel_patch("cpuid",				patch_cpuid_set_info_all,     KERNEL_ANY, CPUID_MODEL_UNKNOWN);
    
    register_kernel_patch("commpage",			patch_commpage_stuff_routine, KERNEL_ANY, CPUID_MODEL_ANY);
    register_kernel_patch("lapic init",			patch_lapic_init,             KERNEL_ANY, CPUID_MODEL_ANY);
    register_kernel_patch("lapic interrupt",	patch_lapic_interrupt,        KERNEL_ANY, CPUID_MODEL_ANY);
    
    // NOTE: following is currently 32bit only
    register_kernel_patch("lapic configure",	patch_lapic_configure,        KERNEL_32,  CPUID_MODEL_ANY);
    
    register_kernel_patch("kext injector",		patch_readStartupExtensions,  KERNEL_ANY, CPUID_MODEL_ANY);
    
    register_kernel_patch("power managment",	patch_pmKextRegister,         KERNEL_ANY, CPUID_MODEL_ANY);
    //register_kernel_patch(patch_pmCPUExitHaltToOff,     KERNEL_ANY, CPUID_MODEL_ANY);
    
    register_kernel_patch("xcpm msr",			patch_xcpm_msr,               KERNEL_ANY, CPU_MODEL_ANY);
    
    register_kernel_patch("kext patches",		patch_kexts,                  KERNEL_ANY, CPU_MODEL_ANY);
...
void patch_kernel(void* kernelData, void* arg2, void* arg3, void *arg4)
{
    ...
    UInt32* version_major = (UInt32*)(UInt32)(ver_major ? kernelData + ver_major->addr - txt->address + txt->offset: 0);
    UInt32* version_minor = (UInt32*)(UInt32)(ver_minor ? kernelData + ver_minor->addr - txt->address + txt->offset: 0);
    UInt32* version_rev   = (UInt32*)(UInt32)(ver_rev   ? kernelData + ver_rev->addr   - txt->address + txt->offset: 0);
    char*   version_str   =   (void*)(UInt32)(version   ? kernelData + version->addr   - txt->address + txt->offset: 0);
    char* start = 0;
    ...
    while(kernelPatch)
    {
        ...
        {
            if(kernelPatch->patchRoutine)
            {
                verbose(HEADER "Calling kernel patch '%s'\n", kernelPatch->name);
                kernelPatch->patchRoutine(kernelData);
                called = true;
            }
        }
        ...
        if(!called)
        {
            verbose(HEADER "Skipping kernel patch '%s'\n", kernelPatch->name);
        }
...
void section_handler(char* binary, char* base, char* section, char* segment, void* cmd, UInt64 offset, UInt64 address)
{
    ...
    if(!kernelSection)
    {
        kernelSection = register_section(segment, section);
    }
